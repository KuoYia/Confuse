...
public class PoetTest {
    ...
    @Test
    public void testDeletePoet1() {
        // 查询表中的最后一行的ID
        int lastId = poetMapper.getLastId();
        if (lastId != -1) {
            // 删除最后一行
            poetService.delete(lastId);
        } else {
    ...
    @Test
    public void testDeletePoet3() {
        // 执行删除操作并接收返回值
        boolean deletionResult = poetService.delete(39);

        // 验证删除操作是否成功,因为没有id为39的诗人了，所以删除失败
        assertTrue(deletionResult, "Deletion of poet with ID 39 should be successful");

        // 进一步验证诗人是否确实被删除
        Poet deletedPoet = poetService.findById(39);
        assertNull(deletedPoet, "Poet with ID 39 should be null after deletion");

        // 如果删除成功，打印成功信息
        if (deletionResult) {
            System.out.println("Deletion of poet with ID 39 was successful.");
        }
    }
    @Test
